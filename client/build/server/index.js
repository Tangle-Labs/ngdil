import { c as create_ssr_component, i as setContext, v as validate_component, m as missing_component } from './chunks/index-78a5ef04.js';
import { r as readable, w as writable } from './chunks/index2-c56263c2.js';
...
function get_cookies(request, url, dev, trailing_slash) {
  ...
  const initial_cookies = cookieExports.parse(header, { decode });
  ...
  function get_cookie_header(destination, header2) {
    ...
    if (header2) {
      const parsed = cookieExports.parse(header2, { decode });
      ...
      fetch: async (info2, init3) => {
        ...
        if (set_cookie) {
          for (const str of setCookieExports.splitCookiesString(set_cookie)) {
            const { name, value, ...options2 } = setCookieExports.parseString(str);
            event.cookies.set(
              name,
              value,
              options2
