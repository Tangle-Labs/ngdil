...
import { w as peterChosenStudents } from './flows.store-Ch_DhNys.js';
      ...
      },
      {
        default: () => {
          return `<div class="modal-content svelte-bwlrzy"><img src="/imgs/kw1c-white.png" alt="" class="logo svelte-bwlrzy"> ${validate_component(Typography, "Typography").$$render(
            ...
            {
              default: () => {
                return `${` ${escape($_("journeys.peter.about_to_issue_student_courses_badges", { values: { BadgeCount: selected.length } }).toUpperCase())} `}`;
              }
            }
          )} <div class="p svelte-bwlrzy">${`${escape($_("journeys.peter.click_issue_badges_to_issue_selected_students"))}`}</div> ${`<button class="button svelte-bwlrzy">${escape(
            $_("journeys.peter.issue_badges").toUpperCase()
          )}</button>`} <div class="subtext svelte-bwlrzy">${validate_component(Typography, "Typography").$$render($$result, { variant: "sub-text" }, {}, {})}</div></div>`;
        }
      }
    )} <div class="heading svelte-bwlrzy">${validate_component(Typography, "Typography").$$render($$result, { variant: "heading" }, {}, {
      default: () => {
        return `<!-- HTML_TAG_START -->${highlight($_("journeys.peter.all_passed_now_issue_course_badges"))}<!-- HTML_TAG_END -->`;
      }
    })}</div> <div class="sub-text svelte-bwlrzy">${validate_component(Typography, "Typography").$$render($$result, {}, {}, {
      default: () => {
        return `${escape($_("journeys.peter.select_students_and_issue_their_badges"))}`;
      }
      ...
      {},
      {
        default: () => {
          return `<div class="sidebar svelte-bwlrzy">${each(Array(5), (i) => {
            ...
            default: () => {
              return `${escape($_("journeys.peter.course_result"))}`;
            }
          })}</div> ${each($peterChosenStudents, (student) => {
            return `<div class="data svelte-bwlrzy">${validate_component(Typography, "Typography").$$render(
              $$result,
              ...
              {
                default: () => {
                  return `${escape($_("journeys.peter.pass").toUpperCase())}`;
                }
              }
            )} </div>`;
          })}</div></div> <div class="button-container svelte-bwlrzy"><button class="${escape(null_to_empty(`button ${selected.length <= 0 && "disabled"}`), true) + " svelte-bwlrzy"}">${escape($_("journeys.peter.issue_course_badges").toUpperCase())}</button></div></div>`;
        }
      }
    )}</div></div>`;
  } while (!$$settled);
...
//# sourceMappingURL=_page.svelte-rrgkRNaW.js.map
