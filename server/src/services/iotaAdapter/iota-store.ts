...
export class IotaStorage implements Storage {
	...
	async setup() {
		...
		for (const config of configs) {
			...
			this._chainStates.set(config.did as string, ChainState.fromJSON(chainState));
			this._documents.set(config.did as string, Document.fromJSON(config.document));
			const keyMap: Map<string, KeyPair> = new Map(
				JSON.parse(vault).map((k: any) => [k[0], KeyPair.fromJSON(k[1])])
			);
	...
	public async keyPublic(did: DID, keyLocation: KeyLocation): Promise<Uint8Array> {
		...
		if (vault) {
			const keyPair: KeyPair | undefined = KeyPair.fromJSON(vault.get(keyLocation.canonical()));
			if (keyPair) {
	...
	public async flushChanges(): Promise<void> {
		...
		for (const didConfig of didConfigs) {
			...
			await this._storage.findOneAndUpdate({ did: didConfig.did }, { extras, document });
		}
	}
}
