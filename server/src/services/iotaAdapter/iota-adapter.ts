	...
	implements NetworkAdapter<T>
{
	...
	public async createDid<T extends StorageSpec<any, any>>(
		...
		const key = seed
			? KeyPair.tryFromPrivateKeyBytes(KeyType.Ed25519, stringToBytes(seed))
	...
	implements IdentityAccount
{
	...
	public static async build<T extends StorageSpec<Record<string, any>, any>>(
		...
		const key = KeyPair.tryFromPrivateKeyBytes(KeyType.Ed25519, stringToBytes(seed));
		...
		if (!isOld) {
			await storage.findOneAndUpdate({ alias }, { did: did.did().toString() });
	...
	implements CredentialsManager<T>
{
	...
	public async create(props: CreateCredentialProps): Promise<Record<string, any>> {
...
export async function isCredentialValid(cred: Record<string, unknown>): Promise<boolean> {
	const resolver = await Resolver.builder().clientConfig(clientConfig).build();
	const signedVc = Credential.fromJSON(cred);
	const issuerIdentity = await resolver.resolve(DID.parse(signedVc.issuer().toString()));
